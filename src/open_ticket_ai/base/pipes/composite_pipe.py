...
class CompositePipe[ParamsT: StrictBaseModel](Pipe[ParamsT]):
    @staticmethod
    ...
    async def _process_steps(self, context: PipeContext) -> list[PipeResult]:
        ...
        return [await self._process_step(step_config, context) for step_config in self._config.steps]

    @final
    async def _process_step(self, step_config: PipeConfig, context: PipeContext) -> PipeResult:
        step_pipe = self._factory.render(step_config, context.model_dump())
