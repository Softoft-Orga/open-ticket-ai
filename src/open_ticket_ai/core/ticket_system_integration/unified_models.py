...
class UnifiedNote(OpenTicketAIBaseModel):
    ...
    subject: str = Field(
        default="",
        description="Subject or title of the note providing a brief summary of its content."
    )
    body: str = Field(
        default="",
        description="Full text content of the note containing the detailed message or comment."
    )
...
class UnifiedEntity(OpenTicketAIBaseModel):
    ...
    name: str | None = Field(
        default=None,
        description="Human-readable name or label for the entity used for display purposes."
    )
...
class UnifiedTicketBase(OpenTicketAIBaseModel):
    ...
    subject: str | None = Field(
        default=None,
        description="Subject or title of the ticket summarizing the main issue or request."
    )
    ...
    priority: UnifiedEntity | None = Field(
        default=None,
        description="Priority level of the ticket indicating urgency and importance for handling."
    )
...
class UnifiedTicket(UnifiedTicketBase):
    body: str | None = Field(
        default=None,
        description="Main body text of the ticket containing the detailed description or initial message."
    )
...
class TicketSearchCriteria(OpenTicketAIBaseModel):
    ...
    limit: int = Field(
        default=10,
        description="Maximum number of tickets to return in the search results for pagination."
    )
    offset: int = Field(
        default=0,
        description="Number of tickets to skip before returning results for pagination and page navigation."
    )
