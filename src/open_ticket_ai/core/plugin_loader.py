...
class PluginLoader:
    ...
    def parse_plugin_spec(self, plugin_spec: str) -> tuple[str, str | None]:
    ...
    def check_plugin_version(self, package_name: str, version_spec: str | None) -> bool:
        if not version_spec:
            try:
    ...
    def load_plugin(self, plugin_spec: str) -> Any | None:
        ...
        if not self.check_plugin_version(package_name, version_spec):
            if version_spec:
                self.logger.error(f"Plugin '{package_name}' version does not match requirement '{version_spec}'")
            else:
