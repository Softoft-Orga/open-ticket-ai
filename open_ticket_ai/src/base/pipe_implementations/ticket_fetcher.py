...
def ticket_filters_to_search_criteria(filters: list[dict]) -> TicketSearchCriteria:
    """
    ...
    equal_filters = [f for f in filters if f.get('operator') == 'equals']
    ...
    unified_queue_filter = UnifiedQueue(
        name=equal_filters_dict.get('queue_name', None),
        id=equal_filters_dict.get('queue_id', None),
    ) if 'queue_name' in equal_filters_dict or 'queue_id' in equal_filters_dict else None
...
class TicketFetcher(Pipe):
    ...
    async def process(self, context: PipelineContext[dict]) -> PipelineContext[dict]:
        ...
        output_field = self.config.get("output_field", "ticket")  # Default to 'ticket' if not specified
