...
Dies sind nur Testmodelle. Sie können jedes beliebige Modell mit Open Ticket AI verwenden!
...
Installieren Sie nur die benötigten Funktionen über ein **modulares Plugin-System**. Plugins erweitern Open Ticket AI mit benutzerdefinierten Ticket-System-Integrationen, ML-Modellen und Verarbeitungslogik – alles ohne den Kerncode zu berühren.
...
**So funktioniert es:** Plugins sind Standard-Python-Pakete, die über Entry Points entdeckt werden. Installieren Sie mit `uv add otai-otobo-znuny`, verweisen Sie in Ihrer Konfiguration darauf, und Sie sind startklar. Erfahren Sie mehr in der [Plugin-System](../users/plugins.mdx)-Dokumentation.
...
- Bedingte Parameter basierend auf Laufzeitzustand
- Typsichere Konfigurationsschemata

</div>

Dienste werden einmal definiert und über mehrere Pipes hinweg per Dependency Injection wiederverwendet.
Erkunden Sie [Konfiguration & Template-Rendering](../users/config_rendering.mdx) für Details.
...
**2. Erstellen Sie Ihre `config.yml`** (siehe [Konfigurationsleitfaden](../users/config_rendering.mdx))
...
# Open Ticket AI mit allen Plugins installieren
uv pip install open-ticket-ai[all]

# Oder Plugins einzeln installieren
...
Siehe den vollständigen [Installationsleitfaden](../guides/installation.md) für Systemanforderungen und Bereitstellungsoptionen.
...
- **Vollständige Dokumentation**: Kompletter Leitfaden unter [Plugin-Entwicklung](../developers/plugin_development.mdx)
...
Während es heute noch keinen offiziellen Marketplace gibt, arbeiten wir daran:
...
- **Umfassende Tests**: Volle Testabdeckung mit pytest
