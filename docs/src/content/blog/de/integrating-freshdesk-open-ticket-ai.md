---
title: 'Freshdesk-Integration für Open Ticket AI: On-Premise KI-Klassifizierung über REST API'
description: 'Integrieren Sie das selbstgehostete Open Ticket AI mit Freshdesk mithilfe benutzerdefinierter Plugins. Automatisieren Sie die Ticketweiterleitung, Prioritätszuweisung und Queue‑Klassifizierung über REST API.'
lang: en
date: 2025-08-12
tags:
...
Open Ticket AI (OTAI) läuft vollständig on-premise und klassifiziert Support‑Tickets in Warteschlangen, Prioritäten und benutzerdefinierte Kategorien. Um OTAI mit **Freshdesk** zu integrieren, erstellen Sie ein kleines Plugin, das einen `FreshdeskTicketsystemService` bereitstellt. OTAI lädt diesen Service automatisch und verwendet ihn, um Tickets über die Freshdesk REST API zu lesen und zu aktualisieren.

## Architektur

Eine Ticket‑System‑Integration in OTAI folgt immer dem gleichen Muster:
...
- OTAI ruft den Service am Ende der Pipeline auf und schreibt KI‑Ergebnisse zurück zu Freshdesk

Dies ist identisch zu der Funktionsweise von `otai_zammad`.
...
Fügen Sie dies zu Ihrer OTAI `config.yml` hinzu:
...
1. OTAI ruft Tickets von Freshdesk ab (`find_tickets`)
...
Alles läuft on-premise. Die Authentifizierung bei Freshdesk verwendet Basic Auth mit Ihrem API‑Key.

## Vorteile

- volle Datenkontrolle (OTAI bleibt lokal)
- nahtlose Nutzung der Freshdesk UI und Workflows
- KI‑gesteuerte Weiterleitung ohne die proprietäre KI von Freshdesk
- klare Plugin‑Architektur, identisch zu Zammad / OTOBO / Znuny