...
vorherigen Pipes, führt ihre Logik aus und leitet den aktualisierten Kontext weiter.
...
1. Empfängt den `PipeContext` (enthält Ergebnisse von vorherigen Pipes)
...
Führt seine `steps` **in einer Endlosschleife** aus. Dient für Hintergrund-artige Zyklen. Er macht die
Ergebnisse der Kind-Pipes **nicht** als ein einzelnes Pipe-Ergebnis verfügbar. `registryKey: base:SimpleSequentialOrchestrator`
...
- **`params`**: Parameter der aktuellen Pipe
  ...
  - Für verschachtelte Pipes kann über `parent` auf die Eltern-Pipe verwiesen werden

- **`parent`**: Verweis auf Eltern-Parameter
...
Auf diese Ergebnisse greift man in den Parametern einer anderen Pipe mit diesen Funktionen zu. Derzeit gibt es keine
Möglichkeit, was_skipped oder message zu lesen;
...
Die Pipe `ExpressionPipe` gibt den Wert von `expression` zurück; wenn dieser zu einem `FailMarker` ausgewertet wird, schlägt die
...
Die Pipe `SimpleSequentialRunner` führt `on` aus und nur, wenn dies erfolgreich ist, führt es `run` aus. Es ist ein minimaler
