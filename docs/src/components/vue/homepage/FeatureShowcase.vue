  ...
  <section class="w-full px-4 md:px-10 lg:px-20 xl:px-40 py-24 bg-background-dark">
    <div class="w-full max-w-7xl mx-auto">
      ...
      >
        ...
        <div class="absolute -right-20 -top-20 w-96 h-96 bg-secondary/10 rounded-full blur-[100px]"></div>
        ...
        <div class="flex flex-col lg:flex-row gap-16 items-center">
          ...
          >
            <div v-if="mounted" class="w-full lg:w-1/2 order-2 lg:order-1 z-10">
              <div class="relative group">
                ...
                <Card background="surface-dark" padding="none" class="relative">
                  <img 
                    v-if="imageSrc" 
                    :src="imageSrc" 
                    :alt="imageAlt || title"
                    class="w-full h-auto"
                  />
                  <div v-else class="w-full aspect-video bg-gradient-to-br from-primary/20 to-secondary/20 flex items-center justify-center">
          ...
          <Transition
            ...
            enter-to-class="opacity-100 translate-x-0"
          >
            <div v-if="mounted" class="w-full lg:w-1/2 flex flex-col gap-6 order-1 lg:order-2 z-10">
              <Badge v-if="badge" type="primary" class="w-fit uppercase tracking-wider">
                {{ badge }}
              </Badge>
              
              <h2 class="font-display text-4xl font-bold text-white leading-tight">{{ title }}</h2>
              
              <p class="text-text-dim text-xl leading-relaxed">{{ description }}</p>
                ...
                >
                  ...
                  >
                    ...
                    <div class="flex flex-col gap-1">
                      ...
                      <span v-if="feature.description" class="text-text-dim text-sm">{{ feature.description }}</span>
                    </div>
                  </div>
                </Transition>
              </div>
