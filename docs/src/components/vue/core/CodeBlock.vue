<template>
  <div class="relative group">
    <div class="rounded-xl border border-surface-lighter bg-surface-dark overflow-hidden">
      <div
        v-if="title || language"
        class="flex items-center justify-between border-b border-surface-lighter bg-background-dark px-4 py-2"
      >
        <div class="flex items-center gap-2">
          <span
            v-if="language"
            class="text-xs font-mono text-text-dim uppercase"
          >{{ language }}</span>
          <span
            v-if="title"
        ...
        >
          <component
            :is="copied ? CheckIcon : DocumentDuplicateIcon"
            class="w-4 h-4"
            aria-hidden="true"
          />
          {{ copied ? 'Copied!' : 'Copy' }}
