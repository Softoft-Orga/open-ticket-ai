<template>
  <section class="space-y-4">
    <div
      ...
      class="rounded-lg border border-[color:var(--vp-c-divider)] bg-[color:var(--vp-c-bg-soft)] p-4 text-sm text-[color:var(--vp-c-text-2)]"
    >
      Loading example…
    </div>
    <div
      v-else-if="registry.error.value"
      class="rounded-lg border border-[color:var(--vp-c-danger-1,#f87171)] bg-[color:var(--vp-c-bg-soft)] p-4 text-sm text-[color:var(--vp-c-danger-1,#f87171)]"
    >
    ...
    >
      <template #header>
        <h2 class="m-0 text-2xl font-semibold text-[color:var(--vp-c-text-1)]">
          {{ example.name }}
        </h2>
      </template>
      <div class="space-y-4">
