...
describe('Button Contract', () => {
  it('enforces strict variant types', () => {
    expectTypeOf<ButtonProps['variant']>().toEqualTypeOf<'surface' | 'outline' | 'subtle' | undefined>()
  })

  it('enforces strict tone types', () => {
    expectTypeOf<ButtonProps['tone']>().toEqualTypeOf<'neutral' | 'primary' | 'success' | 'warning' | 'danger' | 'info' | undefined>()
  })
})
```
...
```vue
<!-- BEFORE (hardcoded) -->
<input class="w-full rounded-xl border border-surface-lighter bg-surface-dark px-4 py-3..." />
