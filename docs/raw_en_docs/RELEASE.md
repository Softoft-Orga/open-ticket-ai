...
## Bundle Distribution (Optional)

A meta "bundle" package can pin a tested set of plugin versions:

```toml
[project]
name = "open-ticket-ai-bundle"
version = "1.0.0"
dependencies = [
    "open-ticket-ai==1.0.0",
    "open-ticket-ai-hf-local==1.0.0",
    "open-ticket-ai-otobo-znuny-plugin==1.0.0",
]
```

This provides users a guaranteed-compatible set of packages.
...
- Use post-release version (e.g., 1.0.0.post1)

### Plugin Compatibility Error

If plugin contract tests fail:
1. Check plugin's `core_api` matches Core's `REQUIRED_CORE_API`
2. Update plugin implementation if needed
3. Update dependency range in plugin's `pyproject.toml`

### CI Lock Check Fails

If `uv sync --locked` fails in CI:
...
Common issues:
- Missing `README.md` in package directory
- Invalid Python syntax
- Missing dependencies in `pyproject.toml`
- Incorrect package structure in `[tool.setuptools]`
