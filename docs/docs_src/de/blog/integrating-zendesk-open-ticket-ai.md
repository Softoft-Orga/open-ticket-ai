...
Diese Schritte nutzen die Erweiterbarkeit von OTAI. Die Pipeline wird in der Konfiguration definiert (für das Auslösen der Klassifizierung ist kein REST erforderlich), sodass das einfache Einbinden Ihres Adapters die Pipeline veranlasst, Zendesk als Zielsystem zu verwenden.
...
Wenn der `ZendeskAdapter` eingerichtet ist, wird OTAI ihn bei der Ausführung nahtlos in die Pipeline integrieren. Nachdem Sie beispielsweise Ihre KI-Modelle (Warteschlangen- und Prioritätsprädiktoren) eingerichtet haben, wird der Start des OTAI-Schedulers (z. B. `python -m open_ticket_ai.src.ce.main start`) die Pipeline auslösen. OTAI wird Ihren Adapter als letzten „Modifier“-Schritt verwenden: Nachdem die KI die Attribute für jedes Ticket abgeleitet hat, ruft sie `ZendeskAdapter.update_ticket` auf, um diese Attribute wieder in Zendesk anzuwenden. Der gesamte Prozess bleibt für OTAI transparent – es weiß nur, dass es `update_ticket` auf einer Adapter-Klasse aufruft.
