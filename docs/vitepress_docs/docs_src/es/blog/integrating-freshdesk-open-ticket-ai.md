...
* *Implementar el Adaptador:* Crea una `class` `FreshdeskAdapter` que extienda `TicketSystemAdapter` e implementa `update_ticket`, `find_tickets` y `find_first_ticket`. En estos métodos, utiliza los endpoints de la REST API de Freshdesk (p. ej., `GET /api/v2/tickets` y `PUT /api/v2/tickets/{id}`).
