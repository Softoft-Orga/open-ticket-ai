---
description: 'Découvrez la suite Open Ticket AI : un classificateur sur site, une API hébergée,
  un générateur de données synthétiques et des modèles publics pour automatiser votre flux de travail de tickets de support.'
pageClass: full-page
---
# Aperçu des produits

Utilisez cette page pour voir ce qui est disponible aujourd'hui, ce qui est hébergé par nous, et ce qui est à venir.
**Open Ticket AI** est le produit phare sur site ; les **models** et les **APIs** sont des modules complémentaires optionnels.
...
<Table>
    ...
    <Row>
      ...
      <C>Classificateur de tickets local et open-source (files d'attente & priorité) intégré via des pipelines/adaptateurs.</C>
      <C>✅ Disponible</C>
    ...
    <Row>
      ...
      <C>Modèle de base pour file d'attente/priorité EN.</C>
      <C>🚧 Bientôt disponible</C>
      <C>(sera ajouté ici)</C>
    </Row>
    <Row>
      <C><strong>Langues et attributs supplémentaires</strong></C>
      <C>Modèles pour d'autres langues ; prédictions pour les tags, l'assigné ; première réponse optionnelle.</C>
      <C>🧭 En exploration</C>
      <C>(feuille de route)</C>
    </Row>
    <Row>
      <C><strong>Interface utilisateur web pour le générateur de données</strong></C>
      <C>Interface utilisateur de navigateur superposée au générateur pour les utilisateurs non techniques.</C>
      <C>🧭 En exploration</C>
      <C>(feuille de route)</C>
    </Row>
</Table>

> **Note sur la tarification :** L'**API de prédiction allemande** hébergée est actuellement gratuite. Si la demande augmente trop les coûts d'infrastructure, nous pourrions introduire des limitations de débit ou une tarification. **Open Ticket AI** sur site reste open-source et local.
...
- Classifie la **File d'attente** & la **Priorité** sur les tickets entrants ; architecture de pipeline extensible.
- Se marie bien avec notre **Générateur de données synthétiques** pour un démarrage à froid ou l'équilibrage des classes.
...
- Pour les équipes **sans leurs propres données** où les **files d'attente/priorités de base** correspondent raisonnablement bien.
- Utilisez le **modèle allemand** via notre API hébergée (**gratuit pour l'instant**).
...
- Outil Python pour créer des jeux de données de tickets réalistes et étiquetés (sujet, corps, file d'attente, priorité, type, tags, langue, première réponse).
...
- Plusieurs versions disponibles :
    - **v5 / v4 :** EN & DE, les plus grands et les plus diversifiés.
...
- Attributs supplémentaires : **tags**, **assigné** et génération de **première réponse**.
...
Oui. Entraînez localement avec vos données, ou demandez-nous de générer des données synthétiques qui reflètent vos files d'attente/priorités.
