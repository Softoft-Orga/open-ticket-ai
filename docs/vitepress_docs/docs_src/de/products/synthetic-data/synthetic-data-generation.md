---
description: Generieren Sie mehrsprachige, synthetische Datensätze für Kundensupport-Tickets mit unserem Python-Tool. Bietet eine Graph-Pipeline, KI-Assistenten, detailreiche Felder & Kostenverfolgung.
---
...
- **Pipeline:** Graph aus konfigurierbaren KI-„Knoten“ (Thema → E-Mail → Tags → Paraphrase → Übersetzung → Antwort).
- **Modelle:** Funktioniert mit OpenAI, OpenRouter, Together… (GPT-4, Qwen, LLaMA, etc.).
- **Steuerung:** Integriertes CLI, Dev/Prod-Modi, Kosten- & Token-Tracking mit Währungsübersichten.
- **Lizenz:** Geplante **LGPL**-Veröffentlichung.
- **Benötigen Sie das Tool oder Anpassungen?** → **sales@softoft.de**
:::
...
- **Tags:** 4–8 Domänen-/Themen-Tags pro Ticket
- **Agenten-Antwort:** eine **Erstantwort**, die von einem KI-Assistenten verfasst wurde
...
Verwenden Sie Ihre bevorzugten LLMs:
...
* **Dev- vs. Prod-Modi** zum Wechseln zwischen kleinen Testläufen und der Erstellung vollständiger Datensätze

## Schnellstart

Starten Sie einen Job zur Datensatzerzeugung mit dem integrierten CLI:
...
> In der Praxis werden Sie Prompts anpassen, verschiedene Modelle pro Knoten auswählen und domänenspezifische Zufallstabellen (Warteschlangen, Prioritäten, Geschäftstypen usw.) hinzufügen.

## Ausgabeschema

Häufige Spalten, die Sie in unseren generierten CSV/Parquet-Exporten finden werden:

* `ticket_id` (12–13-stellige Zeichenkette)
* `subject`, `body`
...
* `first_response` (Agenten-Antwort)

## Beispieldatensatz auf Kaggle

Wir haben diesen Generator verwendet, um den öffentlichen Datensatz **Multilingual Customer Support Tickets** zu erstellen, der **Prioritäten, Warteschlangen, Typen, Tags und Geschäftstypen** enthält und sich ideal für das Training von Modellen zur Ticket-Klassifizierung und -Priorisierung eignet.
...
3. mehrsprachige Abläufe zu simulieren.
   Wenn Sie möchten, dass wir maßgeschneiderte Datensätze für Sie generieren (Ihre Domäne/Warteschlangen/Prioritäten/Tags, Ihre Sprachen), bieten wir dies als **Dienstleistung** an.
...
* Der **Synthetic Data Generator** wird voraussichtlich unter der **LGPL** veröffentlicht.
* Wenn Sie frühzeitigen Zugriff, eine private Lizenz oder benutzerdefinierte Änderungen/Erweiterungen wünschen, **senden Sie eine E-Mail an `sales@softoft.de`**, und wir richten es für Sie ein.
...
**Kann ich eigene Felder hinzufügen (z. B. *Geschäftsbereich*, *Auswirkung*, *Dringlichkeit*)?**
...
Verwenden Sie den Dev-Modus (kleine `rows`-Anzahl, niedrigere `max_tokens`), Kostenschwellenwerte und günstigere Modelle für frühe Iterationen. Wechseln Sie zu Ihrer bevorzugten Modellmischung, sobald die Ergebnisse zufriedenstellend sind.