---
description: Erfahren Sie, wie Sie OpenTicketAI mit Zammad für die On-Premise KI-Ticket-Klassifizierung
...
Ein **ZammadAdapter** leitet von dieser Klasse ab und implementiert diese Methoden unter Verwendung der Zammad-API. Er hält typischerweise Konfigurationsdaten (Basis-URL, Anmeldeinformationen), die über eine `SystemConfig` injiziert werden. Zum Beispiel:
...
*Zitat:* Die Basisklasse erfordert diese Methoden. In diesem Beispiel verwenden wir `httpx.AsyncClient` (da die Methoden `async` sind), aber man könnte `requests` in einem synchronen Kontext ähnlich verwenden. Zum Beispiel könnte das Abrufen aller Tickets so einfach sein wie `requests.get(f"{base_url}/api/v1/tickets", auth=(user, pwd))`.
...
*   **KI-Inferenzdienste**: Ihre benutzerdefinierten Warteschlangen-/Prioritäts-Klassifikatoren (z. B. ein HuggingFace-`model`).
...
Der Adapter wird dies dann als Artikel an Zammad POSTen.
